...
def Jacobi(A,n,x0,TOL,N):
    ...
    while k <= N:
        for i in range(0,n):
            sum = 0
            for j in range(0,i):
                sum =  sum + A[i][j]*x0[j]   # the 1st summation from  j=1 to i-1
            s = 0
            for j in range(i+1,n):
                s = s + A[i][j]*x0[j] # the second summmation from j= i+1 to n (NB for jacobi we start from x0[j] for both sum and s)
